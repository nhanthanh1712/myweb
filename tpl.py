...
app = Flask(__name__)
app.config['SECRET_KEY']='Nhan Python-Flask Web App'
...
def search():
    ...
    if status:
        status = int(status)
        projects = db.session.query(models.Project).filter(models.Project.status_id == status).all()
        tasks = db.session.query(models.Task).filter(models.Task.status_id == status).all()
    else:
        projects = db.session.query(models.Project).all()
        tasks = db.session.query(models.Task).all()
    return render_template('search_results.html', projects=projects, tasks=tasks)
